...
An **interface** is a contract of a list of method signatures. Any type with
those methods satisfies it automatically, no `implements` keyword needed.
`shape` requires only `area()`, so both rectangle and circle qualify, and
`getArea()` accepts _any_ shape without knowing which type it got.
**The big win is one function works on every shape which is polymorphism.**
