...
Two small interfaces instead of one. `shape` needs `area()` (a value receiver),
so a plain `rec` satisfies it. `scaler` needs `scale()`, which has a **pointer**
receiver — so only `*rectangle` satisfies it, and you must pass `&rec`, not `rec`.
**The catch: a pointer-receiver method means only a pointer satisfies the interface.**
...
```go
recArea := getArea(rec)   // value satisfies shape
var sc scaler = &rec      // &rec - only a pointer satisfies scaler
sc.scale(3)
```
